...
class GeminiService(BaseLLMService):
    def __init__(self):
        ...
        # In-memory chat session storage. For production, consider using Redis or another persistent store.
        self.chats = {}
        # 실제 함수와 도구 이름을 매핑
    ...
    async def process_query(self, query: str, session_id: str | None = None) -> dict:
        """사용자 쿼리를 받아 LLM과 상호작용하며 작업을 처리합니다. 세션 ID를 통해 대화 기록을 유지합니다."""
        if session_id and session_id in self.chats:
            chat = self.chats[session_id]
        else:
            # Start a new chat and generate a new session ID
        ...
        while response.candidates[0].function_calls:
            ...
            for call in function_calls:
                ...
                if func_name in self.tool_function_map:
                    ...
                    # '초안 생성'은 최종 결과물이므로, LLM과의 대화를 중단하고 결과를 바로 반환합니다.
                    if func_name == "draft_summary_report":
    ...
    async def execute_draft_summary_report(self, cql: str, space_key: str, report_title: str, summary_prompt: str) -> dict:
        """요약 보고서 초안 생성 도구를 실행합니다 (검색 -> 요약)."""
        # 1. 페이지 검색
        print(f"보고서 초안 생성을 위해 페이지 검색 중... (CQL: {cql})")
        ...
        # 4. 페이지를 생성하지 않고, 초안 데이터를 담은 딕셔너리를 반환
        print(f"'{report_title}' 제목의 보고서 초안 생성 완료.")
        return {
            "type": "report_draft",
            "space_key": space_key,
            "title": report_title,
